<?php
// Script temporal para importar SPs
...
$procedures = [
    ...
    BEGIN
        ...
        ELSE
            ...
            SET nombres = p_nombres, apellidos = p_apellidos, dni = p_dni,
                telefono = p_telefono, correo = p_correo, estado = p_estado
    ...
    BEGIN
        ...
        END;
        UPDATE Persona SET estado = 'I' WHERE persona_id = p_persona_id;
        IF ROW_COUNT() = 0 THEN
            SET p_resultado = 0;
            SET p_mensaje = 'La persona no existe';
        ELSE
    ...
    BEGIN
        SELECT n.noticia_id, n.titulo, LEFT(n.contenido, 300) AS resumen,
               n.contenido, n.imagen, n.fecha_publicacion,
    ...
    BEGIN
        IF p_solo_activos = 1 THEN
            SELECT comite_id, nombres, apellidos, cargo, foto, orden, estado
            FROM Comite_Directivo WHERE estado = 'A' ORDER BY orden ASC, apellidos ASC;
        ELSE
            SELECT comite_id, nombres, apellidos, cargo, foto, orden, estado
            FROM Comite_Directivo ORDER BY orden ASC, apellidos ASC;
        END IF;
    END",

    // sp_ComiteDirectivo_ObtenerPorId
    "DROP PROCEDURE IF EXISTS sp_ComiteDirectivo_ObtenerPorId",
    "CREATE PROCEDURE sp_ComiteDirectivo_ObtenerPorId(IN p_comite_id INT)
    BEGIN
        SELECT comite_id, nombres, apellidos, cargo, foto, orden, estado
        FROM Comite_Directivo WHERE comite_id = p_comite_id;
    END",
    ...
    BEGIN
        ...
        SET nombres = p_nombres, apellidos = p_apellidos, cargo = p_cargo,
            foto = p_foto, orden = p_orden, estado = p_estado
    ...
    BEGIN
        UPDATE Comite_Directivo SET estado = 'I' WHERE comite_id = p_comite_id;
        IF ROW_COUNT() = 0 THEN
            SET p_resultado = 0;
            SET p_mensaje = 'El miembro del comité no existe';
        ELSE
...
echo "Importando " . (count($procedures) / 2) . " stored procedures...\n";

foreach ($procedures as $i => $sql) {
    try {
        DB::statement($sql);
        if ($i % 2 == 1) {
            echo "✓ SP " . (($i + 1) / 2) . " importado\n";
        }
    } catch (Exception $e) {
...
$total = count(DB::select('SHOW PROCEDURE STATUS WHERE Db = "bdsistemawebgue"'));
echo "\nTotal SPs en base de datos: $total\n";
