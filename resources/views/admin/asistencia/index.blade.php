...
#lectorQr { max-width: 420px; margin: 0 auto; border-radius: .75rem; overflow: hidden; }
#lectorQr video { border-radius: .75rem; }
.escaneos-panel { max-height: 480px; overflow-y: auto; }
</style>
@endpush
...
<div class="container-fluid py-4">
    ...
    @php
        ...
        $modoFisico = request()->has('seccion_id');
    @endphp

    {{-- Pestañas: QR vs físico --}}
    <ul class="nav nav-tabs mb-3" id="asistenciaTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link {{ $modoFisico ? '' : 'active' }}" id="tab-qr-btn" data-bs-toggle="tab" data-bs-target="#tabQr"
                    type="button" role="tab" aria-controls="tabQr" aria-selected="{{ $modoFisico ? 'false' : 'true' }}">
                <i class="bi bi-qr-code-scan me-1"></i>Registrar con QR
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link {{ $modoFisico ? 'active' : '' }}" id="tab-fisico-btn" data-bs-toggle="tab" data-bs-target="#tabFisico"
                    type="button" role="tab" aria-controls="tabFisico" aria-selected="{{ $modoFisico ? 'true' : 'false' }}">
                <i class="bi bi-ui-checks me-1"></i>Registrar físico
            </button>
        </li>
    </ul>
    ...
    <div class="tab-content" id="asistenciaTabsContent">
        ...
        <div class="tab-pane fade {{ $modoFisico ? '' : 'show active' }}" id="tabQr" role="tabpanel" aria-labelledby="tab-qr-btn">
            ...
            <div class="row g-4">
                <div class="col-md-5">
                    <div id="lectorQr" class="bg-dark bg-opacity-10 d-flex align-items-center justify-content-center" style="min-height:280px;"></div>
                </div>
                <div class="col-md-7">
                    <h6 class="fw-semibold mb-2"><i class="bi bi-clock-history me-1"></i>Escaneos de hoy</h6>
                    <div class="escaneos-panel" id="listaEscaneos">
                        <div class="text-muted small escaneo-vacio">Aún no hay escaneos registrados.</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- ============ TAB: FÍSICO ============ --}}
        <div class="tab-pane fade {{ $modoFisico ? 'show active' : '' }}" id="tabFisico" role="tabpanel" aria-labelledby="tab-fisico-btn">
...
@push('scripts')
<script>
window.ASISTENCIA_ESCANEAR_URL = '{{ route("admin.asistencia.escanear") }}';
</script>
@vite(['resources/js/asistencia-qr.js'])
<script>
// Iniciar cámara solo si la pestaña QR es la que está activa al cargar la página
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('tabQr')?.classList.contains('active')) {
        window.iniciarEscanerQr?.();
    }
});
document.getElementById('tab-qr-btn')?.addEventListener('shown.bs.tab', () => window.iniciarEscanerQr?.());
document.getElementById('tab-fisico-btn')?.addEventListener('shown.bs.tab', () => window.detenerEscanerQr?.());
